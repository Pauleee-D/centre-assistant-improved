...
def crawl_website(centre_id, url, output_dir):
    """Crawl a single website and convert to markdown"""
    print(f"Crawling {centre_id}: {url}")
    ...
    try:
        ...
        print(f"  SUCCESS - Saved {len(markdown)} chars")
        return True
...
def main():
    # Output directory
    output_dir = Path('c:/Projects/centre2/data/crawled')
    output_dir.mkdir(exist_ok=True)

    print(f"Starting crawl of {len(CENTRE_WEBSITES)} centre websites...")
    ...
    for i, (centre_id, url) in enumerate(CENTRE_WEBSITES.items(), 1):
        print(f"[{i}/{len(CENTRE_WEBSITES)}] ", end="")
        success = crawl_website(centre_id, url, output_dir)
